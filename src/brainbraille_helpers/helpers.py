...
#   return [dict(i) for i in list(csv.DictReader(open(tsv_file_path), dialect='excel-tab'))]
...
def export_dict_list_to_xsv(file_path, dict_list, delimiter='\t'):
  ...
  content = delimiter.join(columns) + '\n' + '\n'.join([delimiter.join([str(entry[col]) for col in columns]) for entry in dict_list])
