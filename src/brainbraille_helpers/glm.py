...
# TODO:Maybe rewrite all the string parsing using regex? I am very bad with regex
# def identify_template(file_path):
# Identify the template used using file name
# return [str_seg for str_seg in file_path.split('/')[-1].split('_') if str_seg.startswith('space')][0].split('-')[-1]
def extract_bold_file_info_from_name(file_path):
  ...
  sub = [str_segment for str_segment in bold_file_name_segments if str_segment.startswith('sub')][0].split('-')[-1]
  run = [str_segment for str_segment in bold_file_name_segments if str_segment.startswith('run')][0].split('-')[-1]
  task = [str_segment for str_segment in bold_file_name_segments if str_segment.startswith('task')][0].split('-')[-1]
  space = [str_segment for str_segment in bold_file_name_segments if str_segment.startswith('space')][0].split('-')[-1]
  ...
  if not os.path.exists(f'{func_path}/{timeseries_file}.tsv'):
    timeseries_file = f'sub-{sub}_ses-{ses}_task-{task}_run-{run}_desc-confounds_timeseries'
  info = {
      'sub':sub,
      'run': run,
      'ses': ses,
...
def spm_hrf(resolution_s, peak_delay_s = 6, undershoot_delay_s = 16,
           peak_dispersion = 1, undershoot_dispersion = 1,
           peak_undershoot_ratio = 6, onset_s = 0, model_length_s=32):
  time_stamp = np.array(np.arange(0, np.ceil(model_length_s/resolution_s)) - onset_s/resolution_s)
  peak_values = scipy.stats.gamma.pdf(time_stamp, peak_delay_s / peak_dispersion, loc=0, scale= peak_dispersion / resolution_s)
  undershoot_values = scipy.stats.gamma.pdf(time_stamp, undershoot_delay_s / undershoot_dispersion, loc=0, scale = undershoot_dispersion / resolution_s)
  hrf = peak_values - undershoot_values / peak_undershoot_ratio
  return hrf, time_stamp * resolution_s

def spm_d_hrf(resolution_s, peak_delay_s = 6, undershoot_delay_s = 16,
           peak_dispersion = 1, undershoot_dispersion = 1,
           peak_undershoot_ratio = 6, onset_s = 0, model_length_s=32,
           delta_t_s = 1):
  return spm_hrf(resolution_s, peak_delay_s, undershoot_delay_s,
            peak_dispersion, undershoot_dispersion,
            peak_undershoot_ratio, onset_s, model_length_s)[0] - \
            spm_hrf(resolution_s, peak_delay_s, undershoot_delay_s,
            peak_dispersion, undershoot_dispersion,
            peak_undershoot_ratio, delta_t_s, model_length_s)[0]

def spm_dd_hrf(resolution_s, peak_delay_s = 6, undershoot_delay_s = 16,
           peak_dispersion = 1, undershoot_dispersion = 1,
           peak_undershoot_ratio = 6, onset_s = 0, model_length_s=32, d_dispersion = 0.01):
  return (spm_hrf(resolution_s, peak_delay_s, undershoot_delay_s,
          peak_dispersion, undershoot_dispersion,
          peak_undershoot_ratio, onset_s, model_length_s)[0] - \
          spm_hrf(resolution_s, peak_delay_s, undershoot_delay_s,
          peak_dispersion + d_dispersion, undershoot_dispersion,
          peak_undershoot_ratio, onset_s, model_length_s)[0])/0.01

def stimulus_to_stimulus_sequence(stimulus, SPACE_BEFORE_RUN, SPACE_BETWEEN_WORD, SPACE_BETWEEN_PHRASE, SPACE_AFTER_RUN):
  ...
  if (SPACE_AFTER_RUN > SPACE_BETWEEN_PHRASE):
    ...
    stimulus_sequence = stimulus_sequence[0:(SPACE_AFTER_RUN - SPACE_BETWEEN_PHRASE)]
  return stimulus_sequence


MOTION_CONFOUND_LIST = ['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']
MOTION_DERIVATIVE1_CONFOUND_LIST = ['trans_x_derivative1', 'trans_y_derivative1', 'trans_z_derivative1', 'rot_x_derivative1', 'rot_y_derivative1', 'rot_z_derivative1']
MOTION_POW2_CONFOUND_LIST = ['trans_x_power2', 'trans_y_power2', 'trans_z_power2', 'rot_x_power2', 'rot_y_power2', 'rot_z_power2']
MOTION_DERIVATIVE1_POW2_CONFOUND_LIST = ['trans_x_derivative1_power2', 'trans_y_derivative1_power2', 'trans_z_derivative1_power2', 'rot_x_derivative1_power2', 'rot_y_derivative1_power2', 'rot_z_derivative1_power2']
CSF_CONFOUND_LIST = ['csf', 'csf_derivative1', 'csf_derivative1_power2', 'csf_power2']
WM_CONFOUND_LIST = ['white_matter', 'white_matter_derivative1', 'white_matter_derivative1_power2', 'white_matter_power2']

def extract_confound_regressors(confound_dict_list,
                                USE_MOTION = True,
  ...
  if USE_CSF:
    CSF_regressor_indices = len(total_confound_list) + np.arange(len(CSF_CONFOUND_LIST))
    total_confound_list += CSF_CONFOUND_LIST
  if USE_WM:
    WM_regressor_indices = len(total_confound_list) + np.arange(len(WM_CONFOUND_LIST))
    total_confound_list += WM_CONFOUND_LIST
  if USE_MOTION_OUTLIER:
    total_confound_list += [confound_key for confound_key in list(confound_dict_list[0].keys()) if confound_key.startswith('motion_outlier')]
  confound_regressors = np.array([[float(row[confound]) if row[confound] != 'n/a' else float('nan') for confound in total_confound_list] for row in confound_dict_list], dtype=np.float32)
  derivative1_confound_mask = np.array(['derivative1' in confound for confound in total_confound_list], dtype=bool)
  confound_regressors[0, derivative1_confound_mask] = confound_regressors[1, derivative1_confound_mask]
  if USE_CSF:
    confound_regressors[:, CSF_regressor_indices] = confound_regressors[:, CSF_regressor_indices] / np.max(confound_regressors[:, CSF_regressor_indices], axis=0)
  if USE_WM:
    confound_regressors[:, WM_regressor_indices] = confound_regressors[:, WM_regressor_indices] / np.max(confound_regressors[:, WM_regressor_indices], axis=0)

  return confound_regressors.astype(np.float32)

def event_dict_list_to_event_by_type(event_dict_list, trial_types_to_ignore = ['rest'], merge_adjacent=True):
  event_by_type = {}
  for event_dict in event_dict_list:
    ...
    if len(event_by_type[trial_type]):
      ...
      previous_stimulus_end_time_s = previous_event['onset_s'] + previous_event['duration_s']
      # If the previous stimulus extends to the onset of the current, merge the two to one
      if merge_adjacent:
        if previous_stimulus_end_time_s == float(event_dict['onset']):
          event_by_type[trial_type][-1]['duration_s'] += float(event_dict['duration'])
          continue
...
def event_list_to_onset_array(event_list, run_len_s, resolution_s = 0.01, method='nonlinear'):
  ...
  if method == 'constant_impulse':
    ...
    for event in event_list:
      ...
      gam_weight_mask = np.roll(gam_weight_mask, int(onset_time_s - prev_onset_time_s))
...
def dct_drift_basis(frame_timestamps, hf_cut_hz=0.008):
  ...
  order = max(int(np.floor(2 * hf_cut_hz * (frame_timestamps[-1] - frame_timestamps[0]))), 1)
...
def onset_array_convolve_with_basis(onset_array, basis, TR_s, resolution_s=0.01):
  ...
  for i in range(num_basis):
    # output[:, i] = np.convolve(onset_array, basis[:, i])[np.arange(offset, len_onset_array + offset, steps)]
    output[:, i] = np.convolve(onset_array, basis[:, i])[np.arange(0, len_onset_array, steps)[0:num_frames]]
  return output
    ...
    for y in range(y_size):
      for z in range(z_size):
        if mask[x, y, z]:
          ...
          time_series_without_outlier = np.delete(time_series, outlier_index)
          frame_index_without_outlier = np.delete(frame_index, outlier_index)
          interpolated_vals = np.interp(outlier_index, frame_index_without_outlier, time_series_without_outlier).astype(np.float32)
...
def batch_3d_convolve(image, kernel):
  # What the hell is with this 3d convolve thing? Seems to be a lot slower on GPU?
  # convolve_3d_with_kernel = partial(scipy.signal.convolve, in2=kernel, mode='same')
  # with mp.Pool() as p:
    # image_smoothed = p.map(convolve_3d_with_kernel, np.moveaxis(image, 3, 0))

  # convolve_3d_with_kernel = partial(scipy.signal.convolve, in2=kernel, mode='same')
  image_smoothed = Parallel(n_jobs=-1)(delayed(scipy.signal.convolve)(im, in2=kernel, mode='same') for im in np.moveaxis(image, 3, 0))

  return np.stack(image_smoothed, axis=-1)

# def batch_prewhiten_re_estimates(a, regressors, bold_image, error, with_Satterthwaite=True):
#   num_voxel, ar_order = a.shape
#   num_frame, num_regressor = regressors.shape
#   inv_xt_iv_x_out = np.zeros((num_voxel, num_regressor, num_regressor), dtype=np.float32)
...
def batch_prewhiten_re_estimates(a, regressors, bold_image, error):
  ...
  # NOTE: numba does not like hermitian=True, using numba (whithout hermitian) is slightly faster than no numba
  num_voxel, ar_order = a.shape
  num_frame, num_regressor = regressors.shape
  inv_xt_iv_x_out = np.zeros((num_voxel, num_regressor, num_regressor), dtype=np.float32)
  b = np.zeros((num_voxel, num_regressor), dtype=np.float32)
...
def get_mask_range(mask_image):
  ...
  return mask_x_min, mask_x_max, mask_y_min, mask_y_max, mask_z_min, mask_z_max
...
def create_3d_gaussian_smoothing_kernel(fwhm_mm, bold_resolution_x_y_z_mm):
  ...
  if isinstance(bold_resolution_x_y_z_mm, numbers.Number):
    bold_resolution_x_y_z_mm = np.array([bold_resolution_x_y_z_mm, bold_resolution_x_y_z_mm, bold_resolution_x_y_z_mm], dtype=np.float32)
  else:
    bold_resolution_x_y_z_mm = np.array(bold_resolution_x_y_z_mm, dtype=np.float32)
  ...
  image_smoothing_kernel = scipy.stats.multivariate_normal.pdf(pos, cov = sigma * np.eye(3)).astype(np.float32)
  return image_smoothing_kernel

def filter_bold_image(bold_image, mask_image=None, expanded_mask=None, expanded_mask_image_range=None, mask_threshold=0, image_smoothing_kernel = None):
  ...
  # TODO: abstract out the mask calculation and expanded mask calculation so they can be parameters that get passed in
  if mask_image is None:
    print('You sure you don\'t want a mask???? Please just supply a mask to save calculation, a brain mask for your template will do.')
    print('Unless you have a very low resolution or you pre-cropped the image or you have huge RAM (like 128gb+ huge), you will run out of RAM! Just kill the process now!')
    mask_image = np.alltrue(bold_image > mask_threshold, axis=-1)
    expanded_mask = None
    expanded_mask_image_range = None
  if mask_image.dtype != 'bool':
    raise Exception('Something wrong with the mask_image, expect it to be a type bool image')

  ## Since the edge of a convolution is is bad, use an expanded mask so the edge of the real mask is well defined
  if expanded_mask is None:
    ## Calculate the half kernel length of the smoothing kernels to expand the range of the mask
    kernel_len = 0
    if image_smoothing_kernel is not None:
      ...
      if image_smoothing_kernel_len % 2 == 0:
        raise Exception('Use an image_smoothing_kernel_len with odd number of length so it has a center')
      if not np.alltrue(image_smoothing_kernel_len == np.array(image_smoothing_kernel.shape, dtype=int)):
        raise Exception('Invalid image_smoothing_kernel. Make sure the 3 dimensions are the same')
    ...
    if (ar_parameters is None) and (ar_smoothing_kernel is not None):
      ...
      if ar_smoothing_kernel_len % 2 == 0:
        raise Exception('Use an ar_smoothing_kernel_len with odd number of length so it has a center')
      if not np.alltrue(ar_smoothing_kernel_len == np.array(ar_smoothing_kernel.shape, dtype=int)):
        raise Exception('Invalid ar_smoothing_kernel. Make sure the 3 dimensions are the same')
      if ar_smoothing_kernel_len > kernel_len:
        kernel_len = ar_smoothing_kernel_len
    half_kernel_len = int(kernel_len / 2)

    ## grow the mask with a range of half kernel len so the smoothing convolution can behave nicely at the edge of your region of interest
  ...
  if expanded_mask_image_range is not None:
    mask_x_min, mask_x_max, mask_y_min, mask_y_max, mask_z_min, mask_z_max = expanded_mask_image_range
  else:
    mask_x_min, mask_x_max, mask_y_min, mask_y_max, mask_z_min, mask_z_max = get_mask_range(expanded_mask)

  ## Shrink the bold_image and mask_image to only the region of interests
  compact_bold = bold_image[mask_x_min : mask_x_max + 1, mask_y_min : mask_y_max + 1, mask_z_min: mask_z_max + 1, :]
  compact_expanded_mask = expanded_mask[mask_x_min : mask_x_max + 1, mask_y_min : mask_y_max + 1, mask_z_min: mask_z_max + 1]
  compact_mask = mask_image[mask_x_min : mask_x_max + 1, mask_y_min : mask_y_max + 1, mask_z_min: mask_z_max + 1]
...
def glm_with_ar(bold_image, regressors, mask_image = None, expanded_mask = None, expanded_mask_image_range = None, mask_threshold = 0, image_smoothing_kernel = None, ar_smoothing_kernel = None, ar_order=4, ar_parameters=None, estimate_ar_without_whitening=False, use_ar_sse=False):
  # TODO: Add more code for input checks
  # check bold_image size matches regresors matches mask_image
  # TODO: implement robust iterative reweighted least-square
  # TODO: abstract out the mask calculation and expanded mask calculation so they can be parameters that get passed in
  ...
  if ar_parameters is None:
    ...
    if ar_order < 3:
      print('You probably want to use a higher ar_oder unless the scanner is an ancient potato with very low TR')
  else:
    ar_oder = ar_parameters.shape[1]
    ar_smoothing_kernel = None # If ar_parameters is supplied, by default don't do ar parameter spatial smoothing
  ...
  ## For values you expect to do filter on, use compact_expanded_mask, for results in mask you actually care, use compact_mask
  ...
  # print(compact_bold.shape, compact_expanded_mask.shape, compact_mask.shape, mask_in_expanded_mask.shape, np.sum(compact_expanded_mask), np.sum(compact_mask), np.sum(mask_in_expanded_mask))

  ## cache reusable calculation results
  inv_xtx = np.linalg.pinv(regressors.T @ regressors)
  inv_xtx_xt = inv_xtx @ regressors.T

  ## For the actual calculation, only calculated parameters within the compact mask
  ## only need to calculated values in the compact_expanded_mask range when ar
  ## parameter smoothing is required
  if ((ar_order == 0) and (ar_parameters is None)) or (ar_parameters is not None) or (ar_smoothing_kernel is None):
  ...
  if (ar_order == 0) and (ar_parameters is None):
    print('Why would you even do AR=0? Be a good person and model the error autocorrelation!')
    # This is assuming inv(V) is I, so the variance for each sample is assumed
    # to be the same
    sse = e.T @ e
    ...
    if ar_parameters is not None: # if ar_parameters is supplied, use if
      ...
      a[xcorr_not_zero_mask, :] = batch_AR_from_xcorr_using_Yule_Walker(xcorr[xcorr_not_zero_mask, :])
      ...
      if ar_smoothing_kernel is not None:
        ...
        a = batch_3d_convolve(temp_a, ar_smoothing_kernel).reshape((num_voxel, ar_order))[compact_expanded_mask ,:]
    ...
    if use_ar_sse:
      ## This is an alternative to the 'proper' way. Saves computation, but is it the right way?
      ## This seem to inflate the sse by a lot?
      sse = batch_sse_from_xcorr_and_ar(xcorr, a)
      inv_xt_iv_x, b = batch_prewhiten_re_estimates_no_sse(a, regressors, compact_bold[compact_mask, :])
    else:
      inv_xt_iv_x, b, sse = batch_prewhiten_re_estimates(a, regressors, compact_bold[compact_mask, :], e)
...
def prep_bold_info(bold_info):
  ...
  prepped_bold_info['event_by_type'] = event_dict_list_to_event_by_type(prepped_bold_info['event_tsv_content'])
  prepped_bold_info['onset_array_by_type'] = {e_type: event_list_to_onset_array(events, bold_info['run_len_s'], bold_info['resolution_s']) for e_type, events in prepped_bold_info['event_by_type'].items()}
  prepped_bold_info['regressors_by_type'] = {e_type: onset_array_convolve_with_basis(onset_array, bold_info['basis'], bold_info['TR_s'], bold_info['resolution_s']) for e_type, onset_array in prepped_bold_info['onset_array_by_type'].items()}
  regressor_types = sorted(list(prepped_bold_info['regressors_by_type'].keys()))
  prepped_bold_info['regressor_types'] = regressor_types
  # all_stimuli_regressors = np.hstack([regressors_by_type for e_type, regressors_by_type in prepped_bold_info['regressors_by_type'].items()])
  all_stimuli_regressors = np.hstack([prepped_bold_info['regressors_by_type'][regressor_type] for regressor_type in regressor_types])
  timeseries_tsv_content = load_xsv(bold_info['timeseries_tsv_path'])
  prepped_bold_info['timeseries_tsv_raw'] = load_file(bold_info['timeseries_tsv_path'])
  confound_regressors = extract_confound_regressors(timeseries_tsv_content)
  motion_outliers = extract_confound_regressors(timeseries_tsv_content, USE_MOTION = False,  USE_MOTION_DERIVATIVE1 = False,  USE_MOTION_POW2 = False, USE_MOTION_DERIVATIVE1_POW2 = False, USE_CSF = False,  USE_WM = False, USE_MOTION_OUTLIER = True).astype(dtype=bool)
  prepped_bold_info['motion_outlier_index'] = motion_outliers.nonzero()[0]
  prepped_bold_info['all_regressors'] = np.hstack((all_stimuli_regressors, bold_info['dct_regressors'], confound_regressors))
...
def get_cb_with_derivative_boost(b, c, derivative_boost_masks):
  ...
  for i, c_row, derivative_boost_mask in zip(np.arange(len(derivative_boost_masks)), c, derivative_boost_masks):
    ...
    derivative_boost_mask_padded[:, :num_relevant_regressor_col] = derivative_boost_mask
    for j in range(derivative_boost_mask.shape[0]):
      ...
      b_derivative_boost = sign * np.sqrt(np.sum(relevant_b * relevant_b, axis=1)) * np.sum(relevant_c)
...
def prepare_bold_info(input_bold_path, input_event_path, atlas='HOCPAL', desc='th0', image_smoothing_fwhm_mm=2):
  bold_image_handle = nib.load(input_bold_path)
  ## Infer from the file name to get the templateflow template used by fmriperep.
  ...
  regions = [{'region_name': 'left_mask_image', 'vals': [13]}, {'region_name':'right_mask_image', 'vals':[14]}, {'region_name':'mask_image', 'vals':[13,14]}]
  atlas_image_path = templateflow.api.get(template=bold_file_info['space'], resolution=bold_image_resolution_mm[0], atlas=atlas, desc=desc)
  atlas_image_handle = nib.load(atlas_image_path)
  atlas_image = atlas_image_handle.get_fdata(dtype=np.float32).astype(int)
  mask_image_dictionary = { region['region_name']: reduce(np.logical_or, [atlas_image == val for val in region['vals']]) for region in regions}

  full_brain_mask_image_handle = nib.load(bold_file_info['brain_mask_file_path'])
  full_brain_mask_image = full_brain_mask_image_handle.get_fdata(dtype=np.float32).astype(bool)
  mask_image_dictionary = {key: np.logical_and(mask, full_brain_mask_image) for key, mask in mask_image_dictionary.items()}

  mask_image = mask_image_dictionary['mask_image']

  # image_smoothing_kernel = None
  if image_smoothing_fwhm_mm > 0:
    image_smoothing_kernel = create_3d_gaussian_smoothing_kernel(image_smoothing_fwhm_mm, bold_image_resolution_mm)
  else:
    image_smoothing_kernel = None
  ## According to the paper below, the AFNI's ARMA(1) no smoothing works the best.
  ...
  if (image_smoothing_kernel is None) and (ar_smoothing_kernel is None):
    ...
    if (image_smoothing_kernel is not None) and (ar_smoothing_kernel is not None):
      image_smoothing_kernel_len = max(image_smoothing_kernel.shape + ar_smoothing_kernel.shape)
  ...
  resolution_s = 0.01 # use a resoliution higher but devisable by TR_s and stimulus time
  ...
  onset_array_by_type = {e_type: event_list_to_onset_array(events, run_len_s, resolution_s) for e_type, events in event_by_type.items()}
  regressors_by_type = {e_type: onset_array_convolve_with_basis(onset_array, basis, TR_s, resolution_s) for e_type, onset_array in onset_array_by_type.items()}
  regressor_names = []
  ...
  for contrast in contrasts:
    ...
    if np.any(derivative_boost_mask_neg):
      derivative_boost_mask = np.stack((derivative_boost_mask_pos, derivative_boost_mask_neg))
    else:
...
def generate_design_matrix(num_frames, stimulus_timestamps, sf_Hz):
  ...
  for s_i, stimulus in enumerate(stimulus_timestamps):
    ...
    design_matrix[:, s_i * 3: s_i * 3 + 3] = output = onset_array_convolve_with_basis(onset_matrix_high_res[:, s_i], basis, TR_s, resolution_s)[0:num_frames, :]

  return TR_timestamp, design_matrix, dct_regressors

def glm_with_ar_simplified(time_series, regressors, ar_order = 20, use_ar_sse=False):
  inv_xtx = np.linalg.pinv(regressors.T @ regressors)
  inv_xtx_xt = inv_xtx @ regressors.T
  ...
  if (ar_order == 0):
    ...
    if use_ar_sse:
      ...
      ## This seem to inflate the sse by a lot?
      sse = batch_sse_from_xcorr_and_ar(xcorr, a)
      inv_xt_iv_x, b = batch_prewhiten_re_estimates_no_sse(a, regressors, time_series.T)
    else:
      inv_xt_iv_x, b, sse = batch_prewhiten_re_estimates(a, regressors, time_series.T, e)
...
def generate_design_matrix(num_frames, stimulus_timestamps, sf_Hz):
  ...
  return TR_timestamp, design_matrix.astype(np.float32), dct_regressors.astype(np.float32)