...
class HTK_Hmm:
    def __init__(
        self, temp_dir='/tmp/.gt2k_py', num_cpu=None, HTK_PATH=None,
        ...
        use_tied_states=False, num_tied_state_bi_tri_phone_re_estimate=2,
        ...
        self.HTK_PATH = HTK_PATH if HTK_PATH is not None else \
            os.environ.get('HTK_PATH')
        self.SRILM_PATH = SRILM_PATH if SRILM_PATH is not None else \
            os.environ.get('SRILM_PATH')
        ...
        self.num_embedded_training = num_embedded_training if \
            (num_embedded_training > 1) else 1
        self.custom_hmm_obj_for_token_dict = custom_hmm_obj_for_token_dict
        self.custom_hmm_string_for_token_dict = \
            custom_hmm_string_for_token_dict
        ...
        self.embedded_training_HERest_min_var = \
            embedded_training_HERest_min_var
        self.bi_tri_phone_tied_HERest_min_var = \
            bi_tri_phone_tied_HERest_min_var
        self.bi_tri_phone_tied_HERest_min_samp = \
            bi_tri_phone_tied_HERest_min_samp
        ...
        self.bi_tri_phone_edcmd = bi_tri_phone_edcmd if \
            isinstance(bi_tri_phone_edcmd, str) else None
        if self.bi_tri_phone_edcmd is not None:
            self.bi_tri_phone_edcmd = self.bi_tri_phone_edcmd.rstrip()
            if not (self.bi_tri_phone_edcmd.endswith('LC') or
                    self.bi_tri_phone_edcmd.endswith('RC') or
                    self.bi_tri_phone_edcmd.endswith('TC')):
        ...
        if self.bi_tri_phone_edcmd is None:
            ...
            self.bi_tri_phone_edcmd_path = \
                f'{self.temp_path}/mk_{file_name}.hed'
            write_file(
                f'{self.bi_tri_phone_edcmd}\n', self.bi_tri_phone_edcmd_path
            )

        self.use_tied_states = use_tied_states
        self.tied_state_bi_tri_phone_edcmd_path = \
            f'{self.temp_path}/mk_tied_states.hed'
        self.tied_state_bi_tri_phone_edcmd = tied_state_bi_tri_phone_edcmd if \
            isinstance(tied_state_bi_tri_phone_edcmd, str) else None
        ...
        self.bi_tri_phone_mlf_path = \
            f'{self.temp_path}/bi_or_tri_phone_label.mlf'
        ...
        if isinstance(dict_string, str):
            ...
            self.wordlist = list(
                set([
                        e.split(' ')[0]
                        for e in self.dict_string.split('\n') if len(e)
                    ])
            )
    ...
    def _hmm_HResults(
        self, X, y, y_timestamp=None, x_sample_periods_ms=None,
        i_tokens_each_state=3, n_best=1, insertion_penalty=0.0,
        token_label=True, use_nist=False
    ):
        self.predict(
            X, x_sample_periods_ms=None,
            i_tokens_each_state=i_tokens_each_state, n_best=n_best,
            insertion_penalty=insertion_penalty, token_label=token_label
        )
        if y_timestamp is None:
            y_timestamp = [
                [
                    [
                        i*self.label_time_period_ms,
                        (i + 1)*self.label_time_period_ms
                    ] for i in range(len(y_i))
                ] for y_i in y
            ]
        label_file_lab_paths = [
            f'{self.predict_ext_path}/predict_sample_{i + 1}.lab'
        ...
        results = Parallel(n_jobs=self.num_cpu)(
            ...
            for content, label_path, predict_mlf_file_name in
            zip(
                label_contents, label_file_lab_paths,
                self.predict_mlf_file_names
            )
        )
        return results

    def run_HResults_helper(
        self, content, label_path, predict_mlf_file_name, token_label=True,
        use_nist=False
    ):
    ...
    def predict(
        self, X, x_sample_periods_ms=None, i_tokens_each_state=3,
        insertion_penalty=0.0, token_label=True, frame_per_label_is_fixed=False
    ):
        X = np.array([np.array(x_i) for x_i in X])
        is_naive_label_model = (self.bi_tri_phone_edcmd is None) and \
            (not self.use_tied_states) and (self.is_naive_grammar)
        if is_naive_label_model and frame_per_label_is_fixed:
            ...
            X = X.reshape(
                (num_y_labels, self.num_X_frames_per_y_label, X.shape[1])
            )

        raw_result_mlf_content = self.predict_raw(
            X, x_sample_periods_ms=None,
            i_tokens_each_state=i_tokens_each_state, n_best=1,
            insertion_penalty=insertion_penalty, token_label=token_label
        )
        parsed_results = [
    ...
    def predict_raw(
        self, X, x_sample_periods_ms=None, i_tokens_each_state=4, n_best=1,
        insertion_penalty=0.0, token_label=True
    ):
        len_X = len(X)
        predict_ext_file_path = [
            f'{self.predict_ext_path}/predict_sample_{i+1}'
            for i in range(len_X)
        ]
        if x_sample_periods_ms is None:
            x_sample_periods_ms = [
                x_i.shape[0] * self.sample_period_ms for x_i in X
            ]
        results = Parallel(n_jobs=self.num_cpu)(
            delayed(self._export_to_ext)(
                x, ext_file_path, sample_data_period_ms
            ) for x, ext_file_path, sample_data_period_ms in
            zip(X, predict_ext_file_path, x_sample_periods_ms)
        )

        predict_ext_file_names = [
            f'{path}.ext' for path in predict_ext_file_path
        ]
        predict_mlf_file_names = [
            f'{path}.mlf' for path in predict_ext_file_path
        ]
        self.fitted_hmm_model_path = \
            f'{self.predict_ext_path}/fitted_hmm_model'
        ...
        results = Parallel(n_jobs=self.num_cpu)(
            delayed(self._run_hvite)(
                input_data_x_file, output_file, i_tokens_each_state, n_best,
                insertion_penalty, token_label
            ) for input_data_x_file, output_file in
            zip(predict_ext_file_names, predict_mlf_file_names)
        )
        mlf_result_content = Parallel(n_jobs=self.num_cpu)(
            delayed(self._load_hvite_mlf)(name)
            for name in self.predict_mlf_file_names
        )
    ...
    def _run_hvite(
        self, input_data_x_file, output_file, i_tokens_each_state=3,
        n_best=1, insertion_penalty=0.0, token_label=True
    ):
        cmd = f'{self.HTK_PATH}/HVite'
        tokens_path = self.bi_tri_tokens_path \
            if self.bi_tri_tokens is not None else self.tokens_path
        tokens_path = self.tiedlist_path \
            if (self.tiedlist is not None) else tokens_path
        ...
        if n_best > 1:
            param_list = ['-n',  int(i_tokens_each_state), int(n_best)] + \
                param_list
    ...
    def fit(self, X, y, y_timestamp=None, frame_per_label_is_fixed=False):
        ...
        self.tokens = sorted(
            np.unique([label for sent in y for label in sent])
        )
        write_file(
            ''.join([f'{tok}\n' for tok in self.tokens]), self.tokens_path
        )
        if (self.bi_tri_phone_edcmd is None) and \
            (not self.use_tied_states) and \
                (self.is_naive_grammar) and frame_per_label_is_fixed:
            X = np.vstack(X)
        ...
        if self.grammar_string is None:
            words_dictionary = {
                e[0]: e[1:]
                for e in [
                    line.split() for line in self.dict_string.split('\n')
                ]
                if (len(e) > 0)
            }
            self.grammar_string = '$word ='\
                f'{" | ".join(words_dictionary.keys())} ;\n\n( < $word > )'
            write_file(self.grammar_string, self.grammar_path)
        if self.word_lattice_string is None:
            self._hmm_gen_word_lattice_from_grammar()

        default_htk_hmm_obj = self._gen_htk_hmm_obj(
            self.num_states, self.vec_size, self.skip, self.num_mixtures,
            self.use_full_cov
        ) if (self.custom_hmm_def_obj is None) else self.custom_hmm_def_obj
        default_htk_hmm_def_string = \
            self._hmm_def_2_hmm_str(default_htk_hmm_obj)
        token_htk_def_string_dict = {
            t: default_htk_hmm_def_string for t in self.tokens
        }
        # Override default simple hmm def with custom defination if provided
        for token, custom_hmm_obj in \
                self.custom_hmm_obj_for_token_dict.items():
            token_htk_def_string_dict[token] = \
                self._hmm_def_2_hmm_str(custom_hmm_obj)
        for token, custom_hmm_string in \
                self.custom_hmm_string_for_token_dict.items():
        ...
        ext_file_paths = [
            f'{self.ext_path}/sample_{i+1}' for i in range(len(y))
        ]
        sample_data_periods_ms = [sample_i[-1][-1] for sample_i in y_timestamp]
        self.sample_period_ms = np.mean(
            np.array(sample_data_periods_ms) /
            np.array([len(x_i) for x_i in X])
        )

        results = Parallel(n_jobs=self.num_cpu)(
            delayed(self._export_to_ext)(data, data_path, sample_length_ms)
            for data, data_path, sample_length_ms in
            zip(X, ext_file_paths, sample_data_periods_ms)
        )
        ...
        mlf_content = '#!MLF!# \n' + '\n.\n'.join([
            f'"{ext_file_path}.lab"\n' + '\n'.join([
                ...
                for start_end, w in zip(sent_y_timestamp, y_i)
            ]) for ext_file_path, sent_y_timestamp, y_i
            in zip(ext_file_paths, y_timestamp, y)
        ])
        write_file(mlf_content, self.label_mlf_path)

        data_sample_list = [
            f'{ext_file_path}.ext' for ext_file_path in ext_file_paths
        ]
        write_file('\n'.join(data_sample_list), self.sample_list_path)

        Parallel(n_jobs=self.num_cpu)(
            delayed(self._hmm_init_and_isolated_word_train)(tok)
            for tok in self.tokens
        )
        ...
        if self.bi_tri_phone_edcmd is not None:
            ...
            if n_token > max_possible_n:
                raise Exception(
                    'supplied y samples does not support training for'
                    f'{n_token}-tokens. Can support at most {max_possible_n}'
                    '-tokens'
                )
    ...
    def _hmm_bi_tri_tied_state_re_estimate(self):
        current_hmm_num = 4 + self.num_embedded_training + \
            self.num_bi_tri_phone_re_estimate
        t_params = [
        ...
        for i in range(
            current_hmm_num, current_hmm_num +
            self.num_tied_state_bi_tri_phone_re_estimate
        ):
    ...
    def _hmm_convert_to_tied_state_tri_phone(self):
        ...
        if self.tied_state_bi_tri_phone_edcmd is None:
            self.tied_state_bi_tri_phone_edcmd = \
                f'RO {str(self.convert_to_tied_state_threshold)}'\
                f' {self.bi_tri_phone_stats_path}\n'
            for tok in self.tokens:
                self.tied_state_bi_tri_phone_edcmd += f'QS "L_{tok}"'\
                    f' {{{tok}-*}}\nQS "R_{tok}" {{*+{tok}}}\n'
            for state_i in range(2, self.num_states):
                for tok in self.tokens:
                    self.tied_state_bi_tri_phone_edcmd += \
                        f'TB {str(self.TB_threshold)} "ST_{tok}_{state_i}_"'\
                        f' {{("{tok}","*-{tok}+*","{tok}+*","*-{tok}")'\
                        f'.state[{state_i}]}}\n'
            # print(self.tied_state_bi_tri_phone_edcmd)
            self.tied_state_bi_tri_phone_edcmd += \
                f'\nAU {self.bi_tri_tokens_path}\n'\
                f'CO {self.tiedlist_path}\n\nST "trees"\n'
        write_file(
            self.tied_state_bi_tri_phone_edcmd,
            self.tied_state_bi_tri_phone_edcmd_path
        )
        # print(self.tied_state_bi_tri_phone_edcmd)
        current_hmm_num = 3 + self.num_embedded_training + \
            self.num_bi_tri_phone_re_estimate
        triphone_hmm_path = f'{self.temp_path}/hmm.{current_hmm_num}/newMacros'
        tied_triphone_hmm_folder = f'{self.temp_path}/'\
            f'hmm.{current_hmm_num + 1}'
        ...
        self._run_subprocess_helper(cmd, param_list)
        self.tiedlist = [
            tied_tok for tied_tok in load_file(self.tiedlist_path).split('\n')
            if (len(tied_tok) > 0)
        ]
    ...
    def _hmm_convert_to_bi_tri_phone(self):
        hhed_cmd = self._hmm_make_hhed_cmd(
            self.tokens, self.bi_tri_tokens_path
        )
        self.convert_bi_tri_hhed_cmd_path = f'{self.temp_path}'\
            '/convert_bi_tri_hhed_cmd.hed'
    ...
    def _run_subprocess_helper(self, command, input_params):
        input_params = [
            p if isinstance(p, str) else str(p) for p in input_params
        ]
        param_list = [command]
        if self.PRINT_CMD:
            param_list.append('-A')
        ...
        result = subprocess.run(
            param_list + input_params, stdout=subprocess.PIPE,
    ...
    def _export_to_ext(self, data, data_path, sample_length_ms):
        ...
        with open(f'{data_path}.ext', 'wb') as fp:
            ...
            fp.write(
                pack(
                    '>IIHH', num_sample, int(sample_data_period_ms * 10),
                    int(sample_vec_size * 4), 9
                )
            )
            fp.write(pack(f'>{"f" * (data.size)}', *(data.reshape(data.size))))

    def _gen_htk_hmm_obj(
        self, num_states=8, vector_size=1, skip=0, num_mixtures=1,
        use_full_cov=False
    ):
        '''
    ...
    def _hmm_def_2_hmm_str(self, hmm_def):
        ...
        for state_i, state in enumerate(hmm_def['States'], 2):
            ...
            for mix_i, mix in enumerate(state, 1):
                if 'weight' not in mix:
                    mix['weight'] = 1 / mix_i
                hmm_str = f'{hmm_str}\t<Mixture> {mix_i}'\
                    f' {mix["Weight"]:0.6e}\n'
                hmm_str = f'{hmm_str}\t<Mean> {VecSize}\n'
                if VecSize != len(mix['Mean']):
                    raise ValueError(
                        f'length of mean vector ({len(mix["Mean"])}) of state'
                        f' {state_i} mixer {mix_i} does not match definition '
                        f'VecSize: {VecSize}'
                    )
                hmm_str = f'{hmm_str}\t'\
                    f'{" ".join([f"{n:f}" for n in mix["Mean"]])}\n'
                ...
                if 'InvCovar' in mix:
                    ...
                    for invcovar_i, invcovar_r in enumerate(mix['InvCovar']):
                        hmm_str = f'{hmm_str}\t' + \
                            " ".join([
                                f"{n:f}" for n in invcovar_r[invcovar_i:]
                            ]) + '\n'
        ...
        for i in range(1, NumStates - skip - 1):
            hmm_str = f'{hmm_str}\t' + \
                " ".join([
                    f"{n:f}"
                    for n in [0.0] * i + [1 / (skip + 2)] * (skip + 2) +
        ...
        for j in range(1, skip + 1):
            hmm_str = f'{hmm_str}\t' + \
                " ".join([
                    f"{n:f}" for n in
                    [0.0] * (i + j) + [1 / (skip + 2 - j)] * (skip + 2 - j)
                ]) + '\n'
        hmm_str = f'{hmm_str}\t' + \
            f'{" ".join([f"{n:f}" for n in [0.0] * NumStates])}\n'
