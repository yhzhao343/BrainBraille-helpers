...
def information_transfer_per_selection(prior, cm, X = None, Y = None):
  ...
  HY = np.sum([prior_X[i] * normalized_cm[letters_to_ind[X[i]]][letters_to_ind[Y[j]]] * np.log2(normalized_cm[letters_to_ind[X[i]]][letters_to_ind[Y[j]]]) if (normalized_cm[letters_to_ind[X[i]]][letters_to_ind[Y[j]]] != 0.0) else 0 for j in range(M) for i in range(N)])
  HYX = 0
  for j in range(M):
    s = np.sum([prior_X[i] * normalized_cm[letters_to_ind[X[i]]][letters_to_ind[Y[j]]] for i in range(N)])
    if s != 0:
      HYX += s * np.log2(s)
  return HY - HYX
